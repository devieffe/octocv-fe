...
const Questionnaire = () => {
  ...
  }, [quiz?.completed, submitAnswers]); 

  if (!quiz || !quiz.questions) {
    return <div className="text-left text-gray-700">Loading...</div>;
  }
  ...
  return (
    <div className="min-h-screen flex flex-col items-start justify-center px-4 bg-white">
      <div className="max-w-lg w-full space-y-6">
        {!completed ? (
          <>
            <h2 className="text-3xl font-bold text-blue-950 text-left">
              Question {currentQuestionIndex + 1}:
            </h2>
            <h3 className="text-2xl text-left text-gray-800">
              ...
              {currentQuestion.options.map((optionLabel, index) => {
                ...
                return (
                  <div
                    ...
                    className={`cursor-pointer w-full p-4 rounded-lg border shadow transition text-left text-lg
                      ${
                        isSelected
