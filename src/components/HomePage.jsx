...
const Hero = () => (
  ...
  >
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full py-8 sm:py-14 lg:py-0">
      <div className="flex flex-col lg:grid lg:grid-cols-2 gap-8 lg:gap-8 items-center">

        {/* Left — copy */}
        <motion.div variants={stagger} initial="hidden" animate="visible" className="space-y-5 sm:space-y-7 w-full">
          ...
          <motion.h1
            id="hero-heading"
            variants={fadeUp}
            className="text-4xl sm:text-6xl lg:text-7xl font-black text-white leading-[1.04] tracking-tight"
          >
            Land your dream<br />
            job with{" "}
          ...
          <motion.p variants={fadeUp} className="text-lg sm:text-xl text-gray-400 leading-relaxed max-w-full lg:max-w-lg">
            AI-optimized CVs, personalised career paths, and skills assessments —
            everything you need to accelerate your career.
          </motion.p>

          <motion.div variants={fadeUp} className="lg:hidden relative flex justify-center w-full py-1" aria-hidden="true">
            <div className="relative w-[min(72vw,300px)] h-[min(72vw,300px)] bg-red-500/10 rounded-full border border-red-500/10 overflow-hidden flex items-center justify-center">
              <img
                src={myImage}
                alt=""
                width={300}
          ...
          <motion.div variants={fadeUp} className="flex flex-wrap gap-3 sm:gap-4">
            <Link
              to="/signup"
              className="inline-flex items-center gap-2 px-5 py-3 sm:px-8 sm:py-4 text-sm sm:text-base bg-red-600 hover:bg-red-500 text-white font-semibold rounded-xl transition-all duration-200 hover:shadow-lg hover:shadow-red-500/25 hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2"
            >
              Start for free <ArrowRight size={18} aria-hidden="true" />
            </Link>
            <a
              href="#how-it-works"
              className="inline-flex items-center gap-2 px-5 py-3 sm:px-8 sm:py-4 text-sm sm:text-base bg-white/5 hover:bg-white/10 text-white font-semibold rounded-xl border border-white/10 transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50"
            >
              See how it works
            </a>
          </motion.div>
        ...
        <motion.div
          ...
          className="relative hidden lg:flex justify-center lg:justify-end order-2 lg:order-none w-full"
          aria-hidden="true"
        >
          <div className="relative">
            <div className="absolute inset-0 bg-red-500/15 rounded-full blur-2xl scale-110" />
            <div className="relative w-[min(86vw,440px)] h-[min(86vw,440px)] bg-red-500/10 rounded-full border border-red-500/10 overflow-hidden flex items-center justify-center">
              <img
                src={myImage}
      ...
      >
        {STATS.map(({ value, label }) => (
          <motion.div key={label} variants={fadeUp} className="text-center">
            ...
            <dd className="text-3xl sm:text-4xl font-black text-red-500" aria-label={`${value} ${label}`}>
              {value}
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      >
        ...
        <motion.p variants={fadeUp} className="text-gray-400 mt-4 max-w-2xl mx-auto text-lg">
          OctoCV combines AI, career intelligence, and human-centred design to give you
...
const Testimonials = () => (
  <section id="testimonials" aria-labelledby="testimonials-heading" className="py-24 bg-slate-950">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <motion.div
        variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true }}
        className="text-center mb-16"
      >
        ...
        <motion.h2 id="testimonials-heading" variants={fadeUp} className="text-4xl sm:text-5xl font-black text-white">
          Loved by{" "}<span className="text-red-500">professionals</span>
        </motion.h2>
      </motion.div>
      ...
      >
        {TESTIMONIALS.map(({ name, role, stars, text }) => (
          ...
          >
            <div aria-label={`${stars} out of 5 stars`} className="flex gap-0.5">
              {Array.from({ length: stars }).map((_, i) => (
                <Star key={i} size={14} className="fill-red-500 text-red-500" aria-hidden="true" />
              ))}
            </div>
...
const Footer = () => {
  ...
  return (
    <footer aria-label="Site footer" className="bg-slate-950 border-t border-white/5 pt-16 pb-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <nav key={category} aria-label={`${category} links`}>
              ...
              <ul className="space-y-2 list-none m-0 p-0">
                {links.map((link) => (
                  <li key={link}>
                    <a
                      href={link === "Features" ? "#features" : link === "How it works" ? "#how-it-works" : link === "About" ? "#about" : "/"}
          ...
          <nav aria-label="Legal links" className="flex gap-6">
            {["Privacy", "Terms", "Cookies"].map((item) => (
              <a
                ...
                className="text-gray-600 hover:text-gray-400 text-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:rounded"
              >
                {item}
              </a>
            ))}
